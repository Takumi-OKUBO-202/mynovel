...
- **改行・段落の視認性を優先する。** ただし一文ごとに改行するのではなく、**意味のまとまり(一つの場面・話題)ごとに段落を分ける**。さらに、場面転換や話題の切り替わりなど大きな区切りには、通常の改行(空行1つ)より広い空白(空行2つ)を入れ、メリハリをつける。なろう/カクヨムなどスマホ画面で読まれる媒体を意識し、段落を詰め込みすぎない
